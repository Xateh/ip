...
/**
 * Central command handler for Meep. Contains static methods that mutate the in-memory message and
...
class Command {
  ...
  public static boolean addTask(String message) {
    ...
    if (buildPair.getSecond() != null) response.append(buildPair.getSecond().getMessage());
    else {
  ...
  /**
   * Finds tasks whose descriptions contain the given substring and prints the matching tasks in a
   * numbered list. Matching is case-sensitive. If there are no matches, prints a suitable message.
   *
